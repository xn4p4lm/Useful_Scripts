...
echo "yum -y update" | ssh -o ConnectTimeout=1 -o ConnectionAttempts=1 root@$1 'bash -s'
echo "yum -y upgrade" | ssh -o ConnectTimeout=1 -o ConnectionAttempts=1 root@$1 'bash -s'

echo "setting Static and transient Hostname"
echo "hostnamectl set-hostname --static " $2 | ssh -o ConnectTimeout=1 -o ConnectionAttempts=1 root@$1 'bash -s'
echo "hostnamectl set-hostname " $2 | ssh -o ConnectTimeout=1 -o ConnectionAttempts=1 root@$1 'bash -s'
